...
if(isset($_POST['submit']))
{
    ...
    $querySendMail= mysqli_query($con,"select * from tbluser");
    $customer=mysqli_fetch_array($querySendMail);
     
    foreach ($querySendMail as $customer) {
      # code...
        $to = $customer['Email'];
        $subject = "Job hiring at Candidate Recruitment";
        $message = "We are looking for software engineers who master very well Node.js, Vue.js, PostegreSQL ...";
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=utf-8" . "\r\n";
        $headers .= "From: <no-reply@localhost.com>" . "\r\n";
        mail($to,$subject,$message,$headers);

        $successful = $successful.' '.$to;
    }

    echo '<script>alert("mail to :'.$successful.'")</script>';

     $querySelect= mysqli_query($con,"select * from tblvacancy where JobTitle='$jtitle' ");
     $ret=mysqli_fetch_array($querySelect);
     
    foreach ($querySelect as $ret) {
      # code...
      $verifiedData = $ret['ID'];
    }
    if (!isset($verifiedData)) {
                        ...
                        <div class="card-body b-b">
                            ...
                            <div class="input-group">
                                <input type="text" class="date-time-picker form-control"
                                       data-options='{"timepicker":false, "format":"d-m-Y"}' value="01/12/2020"/ id="applydate" name="applydate" required="true">
                            ...
                            <div class="input-group">
                                <input type="text" class="date-time-picker form-control"
                                       data-options='{"timepicker":false, "format":"d-m-Y"}' value="01/11/2021"/ id="lastdate" name="lastdate" required="true">
                                <span class="input-group-append">
                                    <span class="input-group-text add-on white">
                                        <i class="icon-calendar"></i>
                                    </span>
